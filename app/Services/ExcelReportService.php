...
class ExcelReportService
{
    ...
    private function buildDashboard(int $idx): void
    {
        ...
        // ── VBA chart instruction note ────────────────────────────
        $row++;
        $ws->mergeCells("A{$row}:E{$row}");
        $ws->setCellValue("A{$row}", '📊  Charts: open VBA Editor (Alt+F11) → run BuildStatusPieChart & BuildMonthlyBarChart to generate interactive charts from this data.');
        $ws->getStyle("A{$row}")->applyFromArray([
            'font'      => ['italic' => true, 'size' => 9, 'color' => ['argb' => 'FFB45309']],
            'fill'      => ['fillType' => Fill::FILL_SOLID, 'color' => ['argb' => self::GOLD_BG]],
