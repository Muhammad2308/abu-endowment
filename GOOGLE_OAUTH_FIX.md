...
### `donors` Table
All required fields are nullable except:
- `id` (Primary Key)
- `created_at`, `updated_at` (Timestamps)

Fields used by Google OAuth:
- `email` (Unique, Nullable) ✅
- `name` (Nullable) ✅
- `surname` (Nullable) ✅
- `gender` (Nullable) ✅
- `profile_image` (Nullable) ✅
- `donor_type` (Nullable, Default: 'Individual') ✅

### `donor_sessions` Table
- `username` (Unique) ✅
- `google_id` (Unique when not null) ✅
- `auth_provider` (enum: 'email', 'google') ✅
- `google_email`, `google_name`, `google_picture` (Nullable) ✅
...
**Successful Login:**
```
[INFO] Google login attempt: {"email":"user@example.com","google_id":"123456789","name":"John Doe"}
[INFO] Google login: Updated existing donor session: {"session_id":456,"donor_id":123,"email":"user@example.com"}
```

**New User:**
```
[INFO] Google login attempt: {"email":"user@example.com","google_id":"123456789","name":"John Doe"}
[INFO] Google login: Created new donor session: {"session_id":456,"donor_id":123,"email":"user@example.com"}
```

**Error:**
```
[ERROR] Google login error: SQLSTATE[23000]: Integrity constraint violation...
[ERROR] Stack trace: ...
```

## Additional Notes

1. **Google Register vs Google Login:**
   - `google-register`: For first-time users, checks if account exists
   - `google-login`: For returning users, handles both new and existing sessions

2. **Session Management:**
   - Google users are identified primarily by `google_id`
   - Username is set to email for consistency
   - Device sessions enable persistent login

3. **Data Integrity:**
   - Transaction rollback on any error
   - Existing data is preserved and updated, not replaced
   - No duplicate sessions can be created
