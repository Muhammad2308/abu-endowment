<div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">SMS Logs</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Sender ID</th>
                ...
                @forelse ($logs as $log)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $log->recipient_phone }}</div>
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ strlen($log->message) > 45 ? substr($log->message, 0, 45) . '...' : $log->message }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {{ optional($log->sent_at)->format('M d, Y H:i:s') ?? 'N/A' }}
                        </td>
                        ...
                        <div class="sm:flex sm:items-start">
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-2 space-y-2 text-sm text-gray-500 dark:text-gray-400">
                                    <p><strong>Recipient:</strong> {{ $selectedLog->recipient_phone }}</p>
                                    <p><strong>Sender ID:</strong> {{ $selectedLog->sender_id ?? 'ABU' }}</p>
                                    <p><strong>Status:</strong> {{ ucfirst($selectedLog->status) }}</p>
                                    @if ($selectedLog->cost)
                                        <p><strong>Cost:</strong> {{ $selectedLog->cost }}</p>
                                    @endif
                                    @if ($selectedLog->error_message)
                                        <p class="text-red-600"><strong>Error:</strong> {{ $selectedLog->error_message }}</p>
                                    @endif
                                </div>

                                <div class="mt-4 border-t pt-4">
                                    <p class="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Message</p>
                                ...
                                @if ($selectedLog->response_payload)
                                    <div class="mt-4 border-t pt-4">
                                        <p class="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">API Response</p>
                                        <div class="prose dark:prose-invert max-w-none text-sm bg-gray-50 dark:bg-gray-700 p-2 rounded overflow-x-auto">
                                            <pre class="whitespace-pre-wrap">{{ $selectedLog->response_payload }}</pre>
                                        </div>
                                    </div>
                                @endif
                            </div>
